...
class PIAdapter(RunnerAdapter):
    """Runs Pi CLI in headless/print mode via subprocess.

    Parameters
    ----------
    binary:
        Either a path to the ``pi`` executable (str) or a list of
    ...
    async def run(self, node) -> Result:  # type: ignore[override]
        """Invoke ``pi -p <node.spec>`` and return a :class:`Result`."""
