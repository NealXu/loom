...
class CodexAdapter(RunnerAdapter):
    """Runs Codex CLI in headless/print mode via subprocess.

    Parameters
    ----------
    binary:
        Either a path to the ``codex`` executable (str) or a list of
    ...
    def __init__(
        self,
        binary: Union[str, list[str]] = "codex",
        cwd: str | None = None,
    ) -> None:
        self._binary = binary
        self._cwd = cwd
    ...
    async def run(self, node: "Node") -> Result:
        """Invoke ``codex -p <node.spec>`` and return a :class:`Result`."""
